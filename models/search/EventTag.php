 ...
 * EventTag
 *
 * EventTag represents the model behind the search form about `ommu\event\models\EventTag`.
 *
...
use ommu\event\models\EventTag as EventTagModel;
//use ommu\event\models\Events;

class EventTag extends EventTagModel
{
	/**
	 * @inheritdoc
	 */
	...
	{
		return [
			[['id', 'event_id', 'tag_id', 'creation_id'], 'integer'],
			[['creation_date', 'eventTitle', 'creationDisplayname'], 'safe'],
		];
	}
	...
	public function search($params, $column=null)
	{
		if(!($column && is_array($column)))
			$query = EventTagModel::find()->alias('t');
		else
			$query = EventTagModel::find()->alias('t')->select($column);
